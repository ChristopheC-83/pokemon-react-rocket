...
export default function Pokecard({ ref, pokemon, details, ...props }) {
  if (details) {
    return (
      ...
      >
        {/* Infos */}
        <div className="flex flex-col order-2 w-full gap-1 mt-5 text-center md:w-6/12 md:order-1 md:mt-0 md:text-left">
          {/* Pokemon ID */}
          ...
          {/* Pokemon Types */}
          <div className="flex justify-center gap-1 mt-1 text-sm text-white types md:justify-normal">
          ...
          <div className="p-5 mt-5 text-left bg-black rounded-lg bg-opacity-40">
            {/* Pokemon Stats */}
            <h2 className="text-2xl font-semibold text-white">Statistiques</h2>
            <div className="flex flex-col gap-1 mt-4 text-base text-white stats">
        ...
        {/* Image */}
        <div className="relative flex flex-col order-1 md:w-6/12 md:order-2">
          <img
            src={pokemon.sprites.other.home.front_default}
            alt="pokemon"
            className="w-2/3 mx-auto md:w-1/2"
          />
          <div className="inline-flex gap-5 mx-auto mt-1">
            ...
            <div>
              <div
                ...
                className="flex items-center justify-between flex-1 gap-5 px-4 py-3 bg-black bg-opacity-50 rounded-xl"
              >
                <svg
    ...
    <NavLink to={`./pokemon/${pokemon.id}`}>
      ...
      >
        ...
        <div className="flex flex-col w-5/12 gap-1">
          ...
          {/* Pokemon Types */}
          <div className="flex gap-1 text-xs text-white types">
            {pokemon.types.map((type) => (
              <div
                key={`${pokemon.name}-${type.type.name}}`}
                className={`rounded px-1.5 py-1 ${type.type.name} flex items-center gap-1`}
